      ...
      <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 4 }}>
        <button 
          ...
          style={{ 
            flexShrink: 0, 
            padding: '10px 16px', 
            background: selectedDept === 'all' ? '#2563eb' : '#fff', 
            color: selectedDept === 'all' ? '#fff' : '#6b7280', 
            border: `1px solid ${selectedDept === 'all' ? '#2563eb' : '#e5e7eb'}`, 
            borderRadius: 8, 
            fontSize: 13, 
            fontWeight: 500, 
            cursor: 'pointer',
        ...
        {deptTotals.map(d => (
          <button 
            ...
            style={{ 
              flexShrink: 0, 
              padding: '10px 16px', 
              background: selectedDept === d.name ? d.color : '#fff', 
              color: selectedDept === d.name ? '#fff' : '#111827', 
              border: `1px solid ${selectedDept === d.name ? d.color : '#e5e7eb'}`, 
              borderRadius: 8, 
              fontSize: 13, 
              fontWeight: 500, 
              cursor: 'pointer',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                {['Employee', 'Department', 'Role', 'Basic (KSH)', 'Allowances', 'Overtime', 'Deductions', 'Net Pay'].map(h => (
                  <th key={h} style={{ textAlign: 'left', fontSize: 12, color: '#6b7280', fontWeight: 600, padding: '10px 14px', borderBottom: '1px solid #e5e7eb', whiteSpace: 'nowrap' }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr key={row.employeeId} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                  ...
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #f3f4f6', fontSize: 13, color: '#111827' }}>
                    {row.basicSalary.toLocaleString()}
                  </td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #f3f4f6', fontSize: 13, color: '#16a34a' }}>
                    +{row.allowances.toLocaleString()}
                  </td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #f3f4f6', fontSize: 13, color: '#ea580c' }}>
                    {row.overtime > 0 ? `+${row.overtime.toLocaleString()}` : '—'}
                  </td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #f3f4f6', fontSize: 13, color: '#dc2626' }}>
            ...
            <tfoot>
              <tr style={{ background: '#f0f4ff' }}>
                <td colSpan={3} style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700, color: '#111827' }}>
                  Total ({filtered.length} employees)
                </td>
                <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700 }}>
                  {totalBasic.toLocaleString()}
                </td>
                <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700, color: '#16a34a' }}>
                  +{totalAllow.toLocaleString()}
                </td>
                <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700, color: '#ea580c' }}>
                  +{totalOT.toLocaleString()}
                </td>
                <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 700, color: '#dc2626' }}>
                  −{totalDeduct.toLocaleString()}
                </td>
                <td style={{ padding: '12px 14px', fontSize: 14, fontWeight: 800, color: '#2563eb' }}>
