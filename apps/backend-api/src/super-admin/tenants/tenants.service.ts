...
export class TenantsService {
  ...
  async create(dto: CreateTenantDto & {
    ...
    // ✅ Added explicit typing for tenant
    const tenant: { id: string; slug: string } = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.user.create({
        data: {
          ...
          role: 'HOSPITAL_ADMIN', // ✅ Changed to match schema enum
          tenantId: newTenant.id,
          payrollNumber: `EMP-${Date.now()}`, // ✅ Added required field
          devicePin: '0000', // ✅ Added required field
        },
      });
  ...
  async update(id: string, dto: UpdateTenantDto) {
    ...
    const updated = await this.prisma.tenant.update({
      ...
      data: {
        ...
        ...(dto.suspendReason !== undefined && { suspendReason: dto.suspendReason }),
      },
    });
